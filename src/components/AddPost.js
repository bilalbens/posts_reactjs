import React, { useState, useEffect } from 'react'
import './components.css'
import 'bootstrap/dist/css/bootstrap.min.css';
import { useGlobalContext } from '../Context';


export default function AddPost() {
    const {posts,sharePost} =useGlobalContext();

    // const newPostsss= {id:"",name:"",text:"",image:""};

    const [newPost,setnewPost] = useState({id:"",name:"",text:"",image:""})


   



    const handleForm = e=>{
        setnewPost(
            {
            ...newPost,
             [e.target.name]: e.target.value,
            image :"data:image/png;base64,"

            } )

        
    }

  


    
    
    // useEffect(()=>{
    // console.log(newPost)
    // newPost: {temp_id,temp_name,temp_text,temp_image};

    // },[posts])

    // const initialiszPost = ()=>{

    //     setnewPost(
    //         {id:' ',
    //         name:' ',
    //         text:' ',
    //         image:' '
    //     }
    //     )
    // }
   

    let {temp_id,temp_name,temp_text,temp_image} = newPost;
    return (
        <>
        <div className="card mb-3 ms-5 ">
            <div className="row g-0">
                <div className="col">
                    <p className="text-center py-3 border-bottom ">
                        Create Post
                    </p>
                <div className="card-body ">
                    <div className="d-flex align-items-start imaage">
                    <img src="data:image/png;base64," className="rounded-circle" alt="Sample image" />
                    </div>
                    <div className="form">
                        <input onChange={handleForm} name="name" type="text" className="form-control mb-2" id="exampleFormControlInput1" placeholder="You name"  defaultValue={temp_name}/>
                        <textarea onChange={handleForm} name="text" type="text" defaultValue={temp_text} className="form-control" placeholder="What's on your mind ?" id="floatingTextarea2" style={{height:"100px"}}></textarea>
                        <div  className="d-grid gap-2 mt-3">
                            <button className="btn btn-primary" type="button" 
                            onClick={()=> {
                                    
                                    sharePost(newPost)}}
                            >
                            Post</button>
                        </div>
                    </div>
                    
                </div>
                </div>
            </div>
        </div>
        </>



    )
}
